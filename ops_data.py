...
def third_party_delivery_data(filename,month):
    columns_to_use = ['Booking ID','Locality','Status','requested_date','Pickup by','Delivery by','Delivery scheduled time','City']
    df = pd.read_csv(filename, usecols = columns_to_use)
    ...
    DB = ['vlokal bangalore','roadrunnr bangalore','roadrunnr mumbai','shadofax mumbai','parsel mumbai','shadowfax banglore','fayaz','santhosh','dinesh g']
    ...
    fname = 'Output\delivery_data.xlsx'
    output_file = os.path.join(os.path.dirname(filename), fname)
...
def third_party_delivery_data_to_excel(df,sheet_list,writer,key):
    for s in sheet_list:
        df[df[key] == s].to_excel(writer, sheet_name = key+" "+s, index = False)
    return
